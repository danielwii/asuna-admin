export type MetaInfoOptions = {
  ...
  enumData2?: object; // try another way to send enum data
  filterType?: 'Sort';
  /**
   * slash - 根据 / 设定层级结构
   * parent - 根据父类设定层级结构
 ...
 * @constructor
 */
export const MetaInfo = (options: MetaInfoOptions): Function => {
  return function(target, propertyKey: string, descriptor: PropertyDescriptor) {
    target.info = { ...target.info, [propertyKey]: options };
  };
};

export const EntityMetaInfo = (options: EntityMetaInfoOptions): Function => {
  return function(target, propertyKey: string, descriptor: PropertyDescriptor) {
